...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
    ...
    return (
        <Button
            ...
            {...props}
        >
            <ChevronsRight className={cn("h-5 w-5 transition-transform", !isCollapsed ? "rotate-180" : "")} />
...
function SidebarMenuItem({ className, ...props }: React.ComponentProps<"li">) {
    return (
        <li
            ...
            className={cn("group/menu-item relative w-full p-0 m-0 flex items-center justify-center", className)}
            {...props}
        />
    );
}
